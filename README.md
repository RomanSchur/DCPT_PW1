...
![PW_1_4.png](PW_1_4.png)

Після вищеописаних блоків програми, іде перевірка математичної точності даних та усуненню статистичних викидів. Спочатку проводиться аудит колонки total шляхом порівняння фактичних значень із розрахунковими за еталонною формулою (ціна × кількість × знижка). Програма обчислює середню абсолютну помилку (MAE), яка демонструє масштаб розбіжностей, спричинених системними збоями, після чого перезаписує некоректні суми правильними значеннями. Далі виконується фільтрація цін методом міжквартильного розмаху (IQR), що дозволяє видалити аномально високі або низькі значення, які викривляють загальну статистику. На основі очищених даних будується зведена таблиця, що відображає сумарну виручку в розрізі «місто × товар», забезпечуючи базу для коректного порівняльного аналізу продажів.

![PW_1_5.png](PW_1_5.png)

Після перевірка математичної точності даних та усуненню статистичних викидів проводиться чистка рейтингу: значення поза діапазоном [1, 5] анулюються та заповнюються медіаною за категорією товару. Далі, за допомогою кумулятивної суми відсортованих замовлень, визначається дата досягнення екватора річної виручки. Процес збагачення даних реалізується через об'єднання основного масиву з довідником собівартості (merge), що дозволяє виявити товари без цінових відповідників. Завершується блок розрахунком розширеної статистики для топ-5 товарів та математичною нормалізацією цін через NumPy (Min-Max), що приводить їх до єдиного масштабу [0, 1] для подальшого порівняння.

![PW_1_7.png](PW_1_7.png)

Наступний блок фокусується на низькорівневих обчисленнях за допомогою NumPy для глибокого аналізу даних. Спочатку проводиться ідентифікація аномалій методом Z-score, що дозволяє порівняти кількість статистичних викидів із результатами методу IQR. Далі виконується технічна оптимізація пам'яті шляхом конвертації цін у формат float32, з обов'язковою перевіркою точності обчислень. Валідація взаємозв'язків між показниками здійснюється через побудову кореляційної матриці двома способами (Pandas та NumPy) для підтвердження узгодженості результатів. На завершення реалізується модель лінійної регресії через нормальні рівняння лінійної алгебри, що дозволяє розрахувати вплив ціни, кількості та знижки на підсумкову виручку без використання сторонніх бібліотек машинного навчання.

![PW_1_8.png](PW_1_8.png)

Заключний блок програми зосереджений на дослідженні часових закономірностей та верифікації чистоти даних. Для аналізу стабільності продажів реалізовано розрахунок ковзного середнього (Rolling Mean) з вікном у 7 днів, причому результати стандартних методів Pandas порівнюються з високоефективною реалізацією на базі кумулятивних сум NumPy. Наступним кроком є виявлення сезонності: через побудову зведеної таблиці по місяцях та містах визначається регіон із найбільшим розмахом коливань виручки. Завершується програмна реалізація розрахунком інтегрального Індексу якості даних (DQI). Цей показник агрегує частку пропусків, дублікатів та логічних помилок, підтверджуючи успішність проходження всього пайплайну очищення та готовність бази до фінального звітування.

![PW_1_9.png](PW_1_9.png)

## Аналіз результатів:

Успішно усунуті системні помилки, відновивши 3.92% пропущених даних та виправивши середню похибку розрахунків у 128.35. Видалення дублікатів виявило значне штучне завищення виручки (понад 100,000 на кожне велике місто), що дозволило довести підсумковий індекс якості даних (DQI) до 0.9949. Технічна оптимізація забезпечила прискорення обробки у 4.25 раза та економію 25.94 MB пам'яті без втрати точності обчислень.

Аналіз виявив пік продажів у липні та досягнення половини річної виручки вже 1 липня. Регресійна модель підтвердила ключовий вплив ціни на дохід, а порівняння статистичних методів показало, що IQR краще виявляє аномалії в цінах, ніж Z-score. Найбільша волатильність продажів зафіксована в Дніпрі, що вказує на високу сезонність у цьому регіоні.
